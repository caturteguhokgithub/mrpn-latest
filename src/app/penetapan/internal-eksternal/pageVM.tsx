...
import {
  ExsumUrgentDto,
  initExsumUrgentDto,
} from "@/app/executive-summary/partials/tab1Background/cardUrgent/cardUrgentModel";
...
const useUrgensiVM = () => {
  ...
  const [dataUrgensiExsum, setDataUrgensiExsum] = useState<ExsumUrgentDto>({
    ...initExsumUrgentDto,
  });
  const [requestUrgensi, setRequestUrgensi] = useState<doRequestUrgensiDto>({
    ...initUrgensi,
  });

  // Segmen
  const [requestSegmen, setRequestSegmen] = useState<doRequestSegmenDto>({
    ...initUrgensi,
  });
  const [dataSegmen, setDataSegmen] = useState<SegmenResDto>();

  // Swot
  const [requestSwot, setRequestSwot] = useState<doRequestSwotDto>({
    ...initSwot,
  });
